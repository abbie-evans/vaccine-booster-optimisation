...
class Params:
    ...
    def __init__ (self):
        ...
        self.decay_fast = -math.log(2)/35
        self.decay_slow = -math.log(2)/1000
        self.n0_exvacc = 0.22
    ...
    def initial_distribution(self, param):
        ...
        # delay between infectiousness and hospitalisation (Weibull)
            ...
            def weibull_dist_hosp():
                mean_h = self.mean_hosp
                sd_h = self.sd_hosp
                R = sd_h / mean_h

                # equation to solve for k (shape)
                def equation(k):
                    # use mpmath functions
                    return mp.sqrt(mp.gamma(1 + 2 / k) / (mp.gamma(1 + 1 / k) ** 2) - 1) - R

                # solve for k (shape). Provide a robust initial guess
                k = mp.findroot(equation, 1.5)

                # formula to calculate scale so that Weibull mean = mean_h
                lam = mean_h / mp.gamma(1 + 1 / k)

                return float(k), float(lam)

            k, lam = weibull_dist_hosp()
            # numpy.random.weibull returns a sample with scale = 1; multiply by lam to scale
