    ...
    def change_status(self):
        ...
        if self.status == 'exposed':
            ...
            if self.latent_t_i == -1: #if latent period is finished, determine type of infection
                self.determine_status_change(["symptomatic", "asymptomatic"], Params.p_v_symp_a) #determined symptomatic or asymptomatic
                self.infect_t_i = self.pick_distr_prob(self, Params.infectious_time) #determine infectious period time
                if self.status == 'symptomatic': #if symptomatic
                    self.determine_status_change(['symptomatic','hospitalised'], Params.p_nv_IH) #check if they become hospitalised
                    if self.status == 'hospitalised': #if hospitalised, calculate how long in hospital
                        self.hosp_t_i = self.pick_distr_prob(Params.hospitalistion_time)
                        self.determine_status_change(['hospitalised','dead'], Params.p_nv_HD) #check if they die
                        if self.status == 'dead': #if they die, calculate how long it takes
                            self.death_t_i = self.hosp_t_i + self.pick_distr_prob(Params.death_time)
        ...
        if self.status == 'susceptible':
            ...
            if self.status == 'exposed': #once exposed, choose time until infected
                self.latent_t_i = self.pick_distr_prob(Params.latent_time)
